    ...
    <div class="create-content">
      <div class="form-section">
        ...
        <div class="form-item">
          ...
          <t-input v-model="form.location" placeholder="请输入活动地点（线上活动可不填）" clearable />
        </div>
      </div>
      ...
      <div class="form-actions">
        <t-button theme="primary" block size="large" :loading="submitting" @click="handleSubmit">保存并发布</t-button>
        <t-button theme="default" block size="large" style="margin-top: 12px;" @click="handleSaveDraft">保存草稿</t-button>
      </div>
    </div>
  </div>
</template>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    MessagePlugin.success('活动创建成功')
    router.push('/h5/school/activity-list')
  } catch (err) {
    console.error('创建活动失败', err)
    MessagePlugin.error(err.response?.data?.message || '创建活动失败')
  } finally {
...
const handleSaveDraft = async () => {
  ...
  try {
    ...
    MessagePlugin.error(err.response?.data?.message || '保存草稿失败')
  } finally {
    submitting.value = false
  }
}
