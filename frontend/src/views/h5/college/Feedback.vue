<template>
  <div class="h5-college-feedback">
    ...
    <div v-else class="content-wrap">
      ...
      <!-- Feedback list -->
      <div v-if="filteredRecords.length > 0" class="feedback-list">
        <div
        ...
        >
          <div class="item-header">
            <t-tag
              :theme="item.userType === 'STUDENT' ? 'primary' : 'warning'"
              variant="light"
              size="small"
            >
        ...
        <div v-if="hasMore" class="load-more" @click="loadMore">
          <t-loading v-if="loadingMore" text="加载中..." size="small" />
      ...
      <div v-if="currentRecord" class="detail-content">
        ...
        <div class="detail-body">
          ...
          <div v-if="currentRecord.attachments && currentRecord.attachments.length > 0" class="attachment-section">
            ...
            >
              <FileIcon size="16" />
              <span class="file-name">{{ file.name || file.fileName || '附件' + (index + 1) }}</span>
...
import { SearchIcon, FileIcon } from 'tdesign-icons-vue-next'
...
const fetchData = async (append = false) => {
  ...
  try {
    ...
    if (activityFilter.value) {
      res = await feedbackApi.getActivityFeedbacks(activityFilter.value)
    } else {
      res = await feedbackApi.getAllFeedbacks()
    }
    ...
    total.value = res.data?.total || newRecords.length
  } catch (err) {
